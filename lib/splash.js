...
export default function splash({ name, logo, active, total, channel, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' пользователей онлайн из ',
          dom('b.total', total), ' зарегестрированных.'
        ]
        : [dom('b.total', total), ' пользователей зарегестрировалось.']
    ),
    dom('form',
      dom('input type=text placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')),
      dom('button.loading', 'Зарегестрироваться!')
    ),
    dom('p',
      'или ',
      dom('a href=http://https://lambdafrela.slack.com/messages/general/', 'заходи')
    ),
    !iframe && dom('footer',
      'powered by ',
